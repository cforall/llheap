// https://github.com/daanx/mimalloc-bench/blob/master/bench/larson/larson.cpp
...
void _beginthread (VoidFunction x, int, void * z)
{
	...
	if ( pthread_create(&pt, &pa, x, z) ) abort(); // PAB
}
#endif
...
#endif
	printf("runtime (sec): ") ;
	if ( scanf ("%ld", &sleep_cnt) != 1 ) abort(); // PAB

	printf("chunk size (min,max): ") ;
	if ( scanf("%d %d", &min_size, &max_size ) != 2 ) abort(); // PAB
#if defined(_MT) || defined(_REENTRANT)
	//#ifdef _MT
	printf("threads (min, max):   ") ;
	if ( scanf("%d %d", &min_threads, &max_threads) != 2 ) abort(); // PAB
	printf("chunks/thread:  ") ; if ( scanf("%d", &chperthread ) != 1 ) abort(); // PAB
	printf("no of rounds:   ") ; if ( scanf("%d", &num_rounds ) != 1 ) abort(); // pAB
	num_chunks = max_threads*chperthread ;
#else
	printf("no of chunks:  ") ; if ( scanf("%d", &num_chunks ) ) != ! ) abort(); // PAB
#endif
	printf("random seed:    ") ; if ( scanf("%d", &seed ) != 1 ) abort(); // PAB
...
// PAB	double        reqd_space ;
// PAB	ULONG         used_space ;
	int           prevthreads ;
	int           i ;
...
// PAB			reqd_space = (0.5*(min_size+max_size)*num_threads*chperthread) ;
			// used_space = CountReservedSpace() - init_space;
// PAB			used_space = 0;
...
static void * exercise_heap( void *pinput)
{
	...
	for( cblks=0; cblks<pdea->NumBlocks; cblks++){
		...
		*chptr++ = 'a';
		volatile char ch __attribute__(( unused )) /* PAB */ = *((char *) pdea->array[victim]);
		*chptr = 'b';
...
// Local Variables: //
// compile-command: "g++-14 -O3 -Wall -Wextra larson.cc -pthread" //
// End: //
